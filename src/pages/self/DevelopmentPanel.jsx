...
const tone = { MOVING: "text-plum", ACTIVE: "text-plum/60", STALLED: "text-plum/40" };
const bar = { MOVING: "bg-plum", ACTIVE: "bg-olive", STALLED: "bg-plum/30" };
...
function Node({ n }) {
  ...
  return (
    <div className={`absolute ${cls} w-44`}>
      <div className="rounded-2xl border border-plum/25 bg-plum/5 px-4 py-3">
        <p className="text-plum text-base font-semibold">{n.label}</p>
        <p className={`text-[10px] tracking-[0.2em] mt-0.5 ${tone[n.status]}`}>{n.status}</p>
        <div className="mt-2.5 h-1.5 rounded-full bg-plum/10 overflow-hidden">
          <div className={`h-full rounded-full ${bar[n.status]}`} style={{ width: `${n.progress}%` }} />
        </div>
        <p className="text-plum/50 text-[10px] mt-2">{n.sub}</p>
      </div>
    </div>
  );
}
    ...
    >
      <div className="relative w-full max-w-2xl mx-auto h-[420px]">
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 480 420" preserveAspectRatio="none">
          <line x1="240" y1="60" x2="240" y2="210" stroke="rgba(48,23,40,0.25)" strokeWidth="2" />
          <line x1="240" y1="210" x2="240" y2="360" stroke="rgba(48,23,40,0.25)" strokeWidth="2" />
          <line x1="60" y1="210" x2="240" y2="210" stroke="rgba(48,23,40,0.25)" strokeWidth="2" />
          <line x1="240" y1="210" x2="420" y2="210" stroke="rgba(48,23,40,0.25)" strokeWidth="2" />
          <circle cx="240" cy="210" r="44" fill="none" stroke="rgba(48,23,40,0.18)" strokeWidth="1" />
          <circle cx="240" cy="210" r="60" fill="none" stroke="rgba(48,23,40,0.1)" strokeWidth="1" />
        </svg>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
          <span className="block w-7 h-7 rounded-full bg-plum shadow-[0_0_0_12px_rgba(48,23,40,0.12)]" />
          <span className="text-plum/40 text-[9px] tracking-[0.2em] mt-3">NOW</span>
        </div>
